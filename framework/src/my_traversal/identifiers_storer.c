...
struct idval *MakeIdval(void)
{

  struct idval* idval;
  DBUG_ENTER("MakeIdval");

  idval = (struct idval *)MEMmalloc(sizeof(struct idval));
  IDVAL_ID(idval) = NULL;
  IDVAL_VAL(idval) = 1;
...
struct idval *FreeIdval( struct idval *idval)
{
  DBUG_ENTER ("FreeIdval");
...
node *ISdoIdentifiersStorer( node *syntaxtree)
{
  info *arg_info;

  DBUG_ENTER("ISdoIdentifiersStorer");
